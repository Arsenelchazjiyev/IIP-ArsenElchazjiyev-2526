...
class Program
{
    static void Main(string[] args)
    {
		...
		bool isTiener = (leeftijd > 13) && (leeftijd < 20);
		...
		{
			Console.WriteLine("is geen tiener.");
		}
        Console.WriteLine("\n---------------------\n");
		...
		string status = isVerzonden ? "verzonden" : "In Behandeling";
		...
		Console.Write("Ingevoerde Gebruikersnaam = ");
		string ingevoerdeGebruikersnaam = "admin";
		const string verplichteNaam = "admin";
		bool isGeldig = (ingevoerdeGebruikersnaam == verplichteNaam);
		Console.WriteLine($"{isGeldig}");
		...
		char karakter = 'a';
		bool isKlinker = (karakter == 'a') || (karakter == 'e') || (karakter == 'i') || (karakter == 'o') || (karakter == 'u');
		Console.WriteLine($"{karakter}");
		Console.WriteLine($"{isKlinker}");
		...
		Console.Write("Leeftijd = ");
		int leeftijd1 = Convert.ToInt32(Console.ReadLine());
		
		if (leeftijd1 <= 12)
		{
			Console.WriteLine("Kind");
		}
		else if (leeftijd1 >= 13 && leeftijd1 < 17)
		{
         ...
         * Context: Een speler mag door naar het volgende level als ze minimaal 50 punten hebben EN de bazenverslagen is, 
		 OF als ze meer dan 100 punten hebben.
         *
         * Opdracht:
		...
		int huidigePunten = 75;
		bool isBaasVerslagen = true;
		bool magDoorNaarVolgendLevel = (huidigePunten >= 50) && (isBaasVerslagen == true);
         ...
         * 3. Gebruik de `CompareTo()` methode om de twee woorden te vergelijken en sla het resultaat 
		 op in een 'int' variabele 'resultaat'.
		...
		string woord2;
		woord2 = "banana";
		int resultaat = woord1.CompareTo(woord2);
		...
		string merk = "Nike";
		double prijs = 120.0;
		if (merk = "Nike" && prijs > 120.0)
		{
			Console.WriteLine("U krijgt een korting");
		}
		else 
		{
		...
		Console.WriteLine("Geef de temperatuur in: ");
		string temperatuur = Console.ReadLine();
		int temp = int.Parse(temperatuur);
		string temperatuurStatus = (temp > 25) ? "Heet" : "Normaal";
		Console.WriteLine(temperatuurStatus);
		...
		{
			Console.WriteLine("Brons");
		}
		else 
		{
			Console.WriteLine("Geen medaille");
		}
		...
		string thema1 = "drama";
		string thema2 = "geweld";
		
		bool isGeschiktVoorVolwassenen = (thema1 == "geweld" || thema1 == "volwassen" || thema2 == "geweld" || thema2 == "volwassen");
		Console.WriteLine(isGeschiktVoorVolwassenen); 
		...
		bool schakelaar = true;
		if (schakelaar == true)
		{
			Console.WriteLine("Schakelaar is aan");
		}
		else 
		{ 
			Console.WriteLine("Schakelaar is uit");
		}
		...
		int scoreSpeler1 = 1200;
		int scoreSpeler2 = 1500;
		int hoogsteScore = scoreSpeler2 > scoreSpeler1 ? scoreSpeler2 : scoreSpeler1;
		...
		Console.Write("geef je email adres in: ");
		string email = Console.ReadLine();

		
		if (email.Contains("@"))
		{
			Console.WriteLine("Geldige email adres");			
		} 
		else 
		{
			Console.WriteLine();
		...
		bool isOchtend = (nu.Hour < 12);
		if (isOchtend)
		{
			Console.WriteLine("Het is ochtend");
		}
		...
		Console.WriteLine("Ga je akkoord (true/false)");
		string akkoord = Console.ReadLine();
		bool akkoord1 bool.Parse(akkoord);
		if (akkoord)
		{
         ...
         * 3. Controleer met een `if`-statement of de wortel een geheel getal is. Dit kan door te 
		 vergelijken of 'wortel' gelijk is aan `Math.Floor(wortel)`.
		...
		if (wortel == Math.Floor(wortel))
		{
			Console.WriteLine("Het getal is een perfecte vierkant.");
		}
		...
		int punten = 1;
		Console.Writeline(punten > 1 ? "Je hebt" + punten + "punt" : "je hebt" + punten + "punten" );
